...
public final class CooldownCache {
    ...
    private static final String CACHE_SQL = "SELECT `key`, `cooldown` FROM player_data.cooldowns WHERE `uuid` = ?;";
    private static final String SELECT_SQL = "SELECT IFNULL((SELECT `cooldown` FROM player_data.cooldowns WHERE `key` = ? AND `uuid` = ? LIMIT 1), 0);";
    private static final String UPDATE_SQL = "INSERT INTO player_data.cooldowns (`uuid`, `key`, `cooldown`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `cooldown` = ?;";
