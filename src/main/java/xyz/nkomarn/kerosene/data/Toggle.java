...
import xyz.nkomarn.kerosene.data.cache.PlayerCache;
...
public final class Toggle {
    ...
    /**
     * A cache containing the current state of a player's toggles.
     */
    ...
    public static void set(UUID uuid, String key, boolean state) {
        try (Connection connection = PlayerData.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `toggles` (`uuid`, `key`, `state`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `state` = ?;")) {
